...
class AuthorsResource {
  ...
  fun addAuthor(@QueryParam("firstName") firstName: String, @QueryParam("lastName") lastName: String): Response {
    ...
    return Response.ok().location(locationFor(author)).build()
  }
  ...
  fun updateAuthor(@PathParam("authorId") authorId: Int, @RequestBody author: Author) {
    if (authorId != author.id) {
      throw IllegalArgumentException("Author ID from path does not match with author ID from JSON body.")
    }
